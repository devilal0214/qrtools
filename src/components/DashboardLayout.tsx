...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    // Add Home link at the beginning of menu items
    {
      href: '/',
      label: 'Home',
      icon: (
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="h-16 flex items-center justify-between px-4 border-b">
            <Link href="/dashboard/active" className="text-xl font-bold text-blue-600">
              Dashboard
            </Link>
            <button 
          ...
          <nav className="flex-1 overflow-y-auto p-4">
            <ul className="space-y-1">
              {menuItems.map((item) => {
                // Only show menu item if it's always shown or user has the required feature
                if (item.showAlways || (item.requiresFeature && canUseFeature(item.requiresFeature))) {
                  return (
                    <li key={item.href}>
                      <Link
                        href={item.href}
