  ...
  return (
    <AdminLayout>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Customize email templates sent to users. Available placeholders: username, email, link, days
            </p>
          </div>
              ...
              <div key={key} className="space-y-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  <h3 className="text-sm font-semibold text-gray-900 capitalize">
                    {key.replace(/([A-Z])/g, ' $1').trim()} Email
                  </h3>
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div className="space-y-4">
                ...
                {settings.analytics.gpsTrackingEnabled && (
                  <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      <strong>⚠️ Note:</strong> Users must grant location permission. Not all users will accept this request. GPS tracking provides exact latitude/longitude coordinates.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <h4 className="text-sm font-medium text-blue-900 mb-2">📍 Tracking Comparison</h4>
        ...
        {/* Campaign URLs Settings */}
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          <div>
            <h2 className="text-lg font-medium text-gray-900">Campaign URL Tracking</h2>
            <p className="mt-1 text-sm text-gray-500">Configure campaign URL parameters for tracking QR code scans</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <input
                type="checkbox"
                id="campaignEnabled"
                checked={settings.campaignUrls.enabled}
                onChange={(e) => setSettings({
                  ...settings,
                  campaignUrls: { ...settings.campaignUrls, enabled: e.target.checked }
                })}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <label htmlFor="campaignEnabled" className="ml-2 text-sm font-medium text-gray-900">
                Enable Campaign URL Tracking
              </label>
            </div>

            {settings.campaignUrls.enabled && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Tracking Domain (Optional)</label>
                  <input
                    type="text"
                    value={settings.campaignUrls.trackingDomain}
                    onChange={(e) => setSettings({
                      ...settings,
                      campaignUrls: { ...settings.campaignUrls, trackingDomain: e.target.value }
                    })}
                    placeholder="track.yourdomain.com"
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                  />
                  <p className="mt-1 text-xs text-gray-500">Custom domain for shortened tracking URLs</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.campaignUrls.utmSource}
                      onChange={(e) => setSettings({
                        ...settings,
                        campaignUrls: { ...settings.campaignUrls, utmSource: e.target.value }
                      })}
                      placeholder="qrcode"
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.campaignUrls.utmMedium}
                      onChange={(e) => setSettings({
                        ...settings,
                        campaignUrls: { ...settings.campaignUrls, utmMedium: e.target.value }
                      })}
                      placeholder="scan"
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.campaignUrls.utmCampaign}
                      onChange={(e) => setSettings({
                        ...settings,
                        campaignUrls: { ...settings.campaignUrls, utmCampaign: e.target.value }
                      })}
                ...
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h4 className="text-sm font-medium text-blue-900 mb-2">📊 How Campaign URLs Work</h4>
                  <ul className="text-sm text-blue-700 space-y-1">
                    <li>• Automatically append UTM parameters to destination URLs</li>
                    <li>• Track campaign performance in Google Analytics</li>
                    <li>• Example: yoursite.com?utm_source=qrcode&utm_medium=scan&utm_campaign=summer_2025</li>
                  </ul>
                </div>
              </>
            )}
