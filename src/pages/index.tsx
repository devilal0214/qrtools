...
export default function Home() {
  ...
  const qrCallbackRef = useCallback((node: SVGSVGElement | null) => {
    qrRef.current = node;
    if (node) {
      console.log("QR SVG element mounted:", node.tagName, node.viewBox?.baseVal?.width);
    }
  }, []);
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  // Simple download function without save/redirect
        ...
        <div className="grid lg:grid-cols-5 gap-4 sm:gap-6 lg:gap-8 bg-white p-4 sm:p-6 lg:p-7 rounded-[10px]">
          ...
          <div className="lg:col-span-3 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-3xl  p-5 space-y-5">
                ...
                <div className="space-y-4 border border-gray-100 p-5 rounded-[10px]">
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {[
                      ...
                      { id: "soft", label: "Rounded badge" },
                    ].map((f) => (
                      <button
                        key={f.id}
                        onClick={() =>
                          setFrameStyle(f.id as "none" | "card" | "soft")
                        }
                        className={`w-24 h-20 rounded-2xl border flex items-center justify-center text-xs font-medium ${
                          frameStyle === f.id
              ...
              {/* SHAPE TAB */}
              {designTab === "SHAPE" && (
                <div className="space-y-5 border border-gray-100 p-5 rounded-[10px]">
                  <div>
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {[
                        ...
                        <button
                          ...
                          className={`w-20 h-16 rounded-2xl border flex items-center justify-center text-xs font-medium ${
                            style.shape === s.id
                              ? "border-blue-500 bg-blue-50 text-blue-600"
                              : "border-gray-200 bg-gray-50 text-gray-500 hover:bg-gray-100"
                          }`}
                  ...
                  {/* Colours under Shape */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-gray-100 p-5 rounded-[10px]">
                    <div className="space-y-2">
